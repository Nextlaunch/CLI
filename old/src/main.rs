...
fn main() {
    let m = App::new("Next Launch")
        ...
        .version("0.2.31 (Fictitious Phoenix)")
...
fn fetch_latest(client: &Client, url: &str, mut offline: bool) -> (String, Option<Launch>, Vec<Article>, bool) {
    ...
    if launch_response.is_ok() && !offline {
        ...
        if json.results.is_some() {
            ...
            let detail = json.detail.unwrap();
            let msg = detail.split(". ").collect::<Vec<&str>>()[1];
            let tm: Vec<&str> = msg.split("in ").collect();
            let time_res: Result<i64, ParseIntError> = tm[1].split(" seconds.").collect::<Vec<&str>>()[0].parse();
            println!("Due to a throttled initialization, Nextlaunch was unable to fetch any data on the upcoming launches.");

            if let Ok(mut time) = time_res {
                let mut last = Instant::now();
                let (d, h, m, s) = get_remaining(time, false);
                print!("Retrying in {} days, {} hours, {} minutes,  {} seconds     \r", d, h, m, s);
                std::thread::sleep(Duration::from_millis(500));
                while time > 0 {
                ...
                println!();
                let (ip, pl, ar, of) = fetch_latest(client, url, offline);
